...
const (
	AesSaltHeader       = "Salted_By_My_App__" // 18 bytes
...
// encrypt plaintext to hexadecimal encoded ciphertext
func (a *AES) Encrypt(plaintext string) (string, error) {
	...
	// extract key and iv
	key, iv := a.extract(salt[:])

	// get padded plaintext
	padded := a.pkcs7pad([]byte(plaintext))
...
// decrypt hexadecimal encoded ciphertext to plaintext
func (a *AES) Decrypt(ciphertext string) (string, error) {
	...
	unpadded, err := a.pkcs7unpad(padded)
	if err != nil {
		return "", err
	}
...
func (a *AES) extract(salt []byte) (key []byte, iv []byte) {
	...
	for i := 0; i < len(md5Filling)/16; i++ {
		...
		prevSum = md5.Sum(append(append(prevSum[:], a.password...), salt...))
...
func (a *AES) pkcs7pad(data []byte) []byte {
