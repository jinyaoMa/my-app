...
func TestAes(t *testing.T) {
	...
	aes, err := aes.New(aes.Options{
		Base: base.Options{
			AAD:    text,          // additional authenticated data
			Key:    key,           // in base64 w/o prefix
			Iv:     iv,            // in base64 w/o prefix
			Prefix: aes.Alg + "_", // prefix of base64 key
		},
	})
	if err != nil {
		t.Fatalf("new aes failed: %v", err)
	}

	ciphertexts := aes.EncryptBase64s([]string{text})
	plaintexts, err := aes.DecryptBase64s(ciphertexts)
	if err != nil {
