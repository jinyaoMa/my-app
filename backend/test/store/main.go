...
func main() {
	...
	for {
		...
		_, path, _ := s.Cache(checksum+".zip", temp, int64(size), int64(size+uint64(n)), fileInfo.Size(), true)
	...
	if ok {
		...
		for int64(size) <= fileInfo.Size() {
			temp, _ := s.Load(checksum+".zip", int64(size), int64(size+16*store.KB))
