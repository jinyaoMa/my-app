...
type Keyring struct {
	...
	AES        *utils.AesWithSalt `gorm:"-:all"` // aes tool used to encrypt/decrypt
...
/*
func (k *Keyring) Find(db gorm.DB) (ok bool) {
	tx := db.Where(k).Find(k)
	return tx.RowsAffected > 0
}

func (k *Keyring) Create(db gorm.DB) (ok bool) {
	tx := db.Create(k)
...
/*
func (ks *Keyrings) Find(db gorm.DB, selects ...string) (ok bool) {
	var tx *gorm.DB
	if len(selects) > 0 {
...
func (ks *Keyrings) FindByUser(db gorm.DB, userid uint, selects ...string) (ok bool) {
	var tx *gorm.DB
	if len(selects) > 0 {
